...
export class addDeliveriesAndOrders1620660145685 implements MigrationInterface {
    name = 'addDeliveriesAndOrders1620660145685'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query("CREATE VIEW `ProductAvailableView` AS SELECT product_id, (Select SUM(quantity) from ProductSize where product_id = `OrderDetail`.product_id group by ProductSize.product_id) as available, (Select SUM(OrderDetail.quantity) as Reserved from OrderDetail inner join `Order` on OrderDetail.order_id = `Order`.id where `Order`.status = 1 group by OrderDetail.product_id) as reserved, (Select SUM(OrderDetail.quantity) from OrderDetail inner join `Order` on OrderDetail.order_id = `Order`.id where `Order`.status IN (4, 5) group by OrderDetail.product_id) as completed from OrderDetail group by OrderDetail.product_id;");
    }
